...
form.addEventListener('submit',(event)=>{
    ...
    const second_message= `Your Zoddiac sign is ${zodiacSigns[month-1]} `;
    const third_message=compliments[day-1] ;
    
    let idx=Math.floor(Math.random()*20);
    const fourth_message=victimCardCompliments[idx];

    idx=((name.length)*(surname.length)*day)%30;// indx for recomendation
    const fifth_message=recommendations[idx];
   
    idx=(day*month*year)%20;
    const sixth_message=predictions[idx];
    result.innerHTML=`${first_message} , ${second_message}  ${third_message}  ${fourth_message} Our recomendation for you : ${fifth_message} Your future prediction is(${sixth_message}) `
})